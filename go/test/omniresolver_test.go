...
import (
	...
	omni "github.com/voxgig-sdk/my-ip-sdk/go/test/omni"

	vs "github.com/voxgig-sdk/my-ip-sdk/go/utility/struct"
)
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MyIpError); is {
				out := map[string]any{
					"name":    "MyIpError",
...
func omniCtx(arg any, client *sdk.MyIpSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MyIpSDK); is {
			client = live
