...
two agent tools — `my-ip_list` and `my-ip_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/my-ip-mcp)
make build
...
claude mcp add --scope user my-ip \
  -- /absolute/path/to/my-ip-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./my-ip-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// my-ip_list: first page of records
{ "entity": "get_ip_info" }
{ "entity": "get_ip_info", "query": { } }

// my-ip_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/my-ip-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user my-ip \
     -- "$PWD"/dist/*/my-ip-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `my-ip_list` and `my-ip_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_info using my-ip"*
   and it calls `my-ip_list` with `{"entity":"get_ip_info"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `my-ip_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `my-ip_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./my-ip-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"my-ip_load","arguments":{"entity":"get_ip_info","query":{"id":1}}}}'
```
