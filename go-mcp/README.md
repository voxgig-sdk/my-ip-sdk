...
MCP server exposing the MyIp SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `my-ip_list` | `entity`, optional `query` map | First page of records as JSON |
| `my-ip_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `my-ip_list` and
`my-ip_load` tools become available in new sessions.
