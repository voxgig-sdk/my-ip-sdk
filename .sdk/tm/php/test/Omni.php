...
//    only - a typed MyIpContext there would make every ctx
...
final class MyIpOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MyIpContext($clean, null);
    ...
    public static function err_from_map($m): ?MyIpError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MyIpError(is_string($code) ? $code : '', $msg);
    }
